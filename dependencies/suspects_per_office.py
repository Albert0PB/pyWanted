...
def get_field_offices_list(lapse_between_requests: (int, float) = 2):
    current_page = 1
...
def suspects_amount_per_office():
    total_offices_repetitions = get_field_offices_list()
...
def generate_suspects_per_office_files():
    ...
    sorted_offices_info = dict(collections.OrderedDict(sorted(offices_info.items())))
    print(f"\nSaving data files...")
